...
class IsContributorToProject(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        # Vérifie si l'utilisateur actuel est un contributeur du projet associé à l'objet (project/issue/comment).
        return obj.project.contributors.filter(pk=request.user.pk).exists()
